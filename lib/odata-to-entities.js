...
var ODataToEntitiesConverter = /** @class */ (function () {
    function ODataToEntitiesConverter(moduleName) {
        ...
        this.classWithInheritanceTemplate = "export class {className} extends {baseType} { {properties} }";
        ...
        this.MapEdmClasses = function (odataMetadata) {
            ...
            schema.entityTypes.forEach(function (entityType) {
                ...
                if (entityType.baseType) {
                    var namespace = entityType.parent.namespace;
                    var baseType = entityType.baseType.replace(namespace + ".", "");
                    classTemplate = _this.classWithInheritanceTemplate.replace("{baseType}", baseType);
                }
                else {
                    classTemplate = _this.classTemplate;
                }
        ...
        this.MapEdmProperties = function (edmProperties, schema) {
            ...
            edmProperties.forEach(function (edmProperty) {
                ...
                properties = properties + "\n" + _this.propertyTemplate.replace("{property}", edmProperty.name).replace("{type}", propertyType);
            });
            return properties;
        };
